...
<style>
    ...
    .loader-logo { width: 220px; height: auto; animation: pulse-logo 1.5s infinite ease-in-out; }
    .loader-text { margin-top: 20px; font-family: 'Poppins', sans-serif; font-weight: 600; color: #002060; letter-spacing: 1px; text-transform: uppercase; display: flex; align-items: center; }
    .loader-text::after { content: ''; animation: typing-dots 1.5s infinite; width: 15px; text-align: left; }
    @keyframes typing-dots { 0%, 100% { content: ''; } 25% { content: '.'; } 50% { content: '..'; } 75% { content: '...'; } }
    @keyframes pulse-logo { 0% { transform: scale(0.9); opacity: 0.8; } 50% { transform: scale(1.05); opacity: 1; } 100% { transform: scale(0.9); opacity: 0.8; } }
    ...
    .blog-hero h1 { font-size: 2.2rem; font-weight: 700; margin-bottom: 10px; }
    ...
    .blog-card .card-title a:hover { color: #e30613; }
    .blog-card .card-desc {
        font-size: 0.9rem;
        color: #6b7280;
        line-height: 1.5;
    ...
    .blog-card .card-meta .author { font-weight: 500; color: #374151; }
    ...
    .blog-sidebar-box ul li { margin-bottom: 8px; }
    ...
    .blog-sidebar-box ul li a:hover { color: #e30613; }
    ...
    .blog-latest-item .latest-info a:hover { color: #e30613; }
    .blog-latest-item .latest-info span { font-size: 0.75rem; color: #9ca3af; }
    ...
    .blog-pagination .page-link:hover { background: #e30613; color: #fff; border-color: #e30613; }
    .blog-pagination .page-item.active .page-link { background: #e30613; color: #fff; border-color: #e30613; }

    body.dark-only .blog-card { background: #1f2937; border-color: #374155; }
    body.dark-only .blog-card .card-title { color: #f3f4f6; }
    body.dark-only .blog-card .card-title a:hover { color: #e30613; }
    body.dark-only .blog-card .card-desc { color: #9ca3af; }
    body.dark-only .blog-sidebar-box { background: #111827; border-color: #374558; }
    body.dark-only .blog-sidebar-box h5 { color: #f3f4f6; }
    body.dark-only .blog-sidebar-box ul li a { color: #d1d5db; }
    body.dark-only .blog-card .card-meta .author { color: #d1d5db; }
    body.dark-only .blog-hero { background: linear-gradient(135deg, #111827 0%, #1f2937 100%); }
</style>
                                    ...
                                    <div class="col-lg-6 col-md-6">
                                        <div class="blog-card">
                                            <a href="{{ route('blog_article_by_url', $article->url) }}">
                                                <img src="{{ $article->imagen }}" alt="{{ $article->title }}" class="card-img">
                                            </a>
                                            <div class="card-body">
                                                @if($article->category)
                                                ...
                                                @endif
                                                <h3 class="card-title">
                                                    <a href="{{ route('blog_article_by_url', $article->url) }}">{{ $article->title }}</a>
                                                </h3>
                                                <p class="card-desc">{{ Str::limit(strip_tags($article->short_description ?? $article->content_text), 120) }}</p>
                                                <div class="card-meta">
                                                    <span class="author">
                                                        @if($article->author)
                                                    ...
                                                    <span>
                                                        <i class="fa fa-calendar me-1"></i>{{ \Carbon\Carbon::parse($article->created_at)->format('d M Y') }}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
    ...
    details.blog-cat .ver-mas-btn {
        display: block; padding: 8px 0 0 0; font-size: 0.8rem; font-weight: 600;
        color: #e30613; cursor: pointer; border: none; background: none; text-decoration: none;
    }
        ...
        $(document).ready(function() {
            ...
            setTimeout(function() {
                $('.loader-wrapper').fadeOut('slow', function() {
                    $(this).remove();
                    if (window.AOS !== undefined) { AOS.refresh(); }
                });
            }, 2500);
        });
    </script>
