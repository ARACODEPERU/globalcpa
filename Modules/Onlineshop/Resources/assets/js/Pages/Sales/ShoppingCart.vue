<script setup>
    ...
    const initMercadoPago = async () => {
        ...
        try {
            // Realiza la solicitud POST para crear la preferencia en MercadoPago
            const response = await axios({
                method: 'post',
                url: route('academic_create_items_preference_course'),
            ...
            if (response.data.success) {
                ...
                // Inicializa MercadoPago con el preference_id
                mp.value = new window.MercadoPago(publicKey.value, { locale: "es" });

                // Obtiene el builder de Bricks
                const bricksBuilder = mp.value.bricks();

                // Renderiza el brick de pago
                await renderCardPaymentBrick(bricksBuilder, response.data.preference_id);
            } else {
                console.error("Error al crear la preferencia de MercadoPago:", response.data.message);
            }
        } catch (error) {
    ...
    const removeItemShoppingCart = (id) => {
        store.removeFromCart(id);
        router.visit(route('onlineshop_sales_shoppingcart', props.payTipe), {
            method: 'get',
            replace: true,
            preserveState: false,
            preserveScroll: false,
            ...
            <div class="grid grid-cols-6 gap-6">
                            ...
                            <div class="space-y-4 md:flex md:items-center md:justify-between md:gap-6 md:space-y-0">
                                ...
                                <div class="w-full min-w-0 flex-1 space-y-4 md:order-2 md:max-w-md">
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <button @click="removeItemShoppingCart(item.id)" type="button" class="inline-flex items-center text-sm font-medium text-red-600 hover:underline dark:text-red-500">
                                            <svg class="me-1.5 h-5 w-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                ...
                <div class="col-span-6 sm:col-span-2">
                    <div class="space-y-4 rounded-lg border border-gray-200 bg-white p-4 shadow-sm dark:border-gray-700 dark:bg-gray-800 sm:p-6">
                        ...
                        <Link :href="route('onlineshop_sales_shoppingcart','me')" class="flex w-full items-center justify-between rounded-lg bg-blue-700 px-5 py-2.5 text-sm font-medium text-white hover:bg-blue-800 focus:outline-none focus:ring-4 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
