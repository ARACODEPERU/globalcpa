<script  setup>
    ...
    const openCertificatePreview = async () => {
        ...
        try {
            const response = await axios.get(route('aca_module_certificate_download', props.module.id), {
                params: { preview: 1 },
            });
            ...
            const title = error.response?.status === 404 ? 'Certificado no encontrado' : 'Error al cargar';
            const message = error.response?.data?.message || 'No se pudo cargar el certificado.';
            Swal2.fire({
                icon: 'error',
                title: title,
                text: message,
