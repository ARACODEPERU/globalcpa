...
class AcaSalesController extends Controller
{
    ...
    public function spaceSalesList($id)
    {
        $affectations = DB::table('sunat_affectation_igv_types')->get();
        $unitTypes = DB::table('sunat_unit_types')->get();
        $student = AcaStudent::with('person')->where('id', $id)->first();

        return Inertia::render('Academic::Students/ListSpaceSales', [
            'affectations'  => $affectations,
            'unitTypes'     => $unitTypes,
            'taxes'         => array(
                'igv' => $this->igv,
    ...
    public function tableSpaceSalesList($id)
    {
        $sales = (new Sale())->newQuery();
    ...
    public function spaceSalesCreate($id){
        ...
        return Inertia::render('Academic::Sales/CreateFeeDocument', [
            'payments' => $payments,
    ...
    public function storeSpacePayments(Request $request)
    {
        ///se validan los campos requeridos
        ...
        try {
            $res = DB::transaction(function () use ($request) {
                ...
                $total_discount = 0;
                $total = 0;
                $nextPaymentDate = $request->get('nextPaymentDate');
                ...
                foreach ($products as $produc) {
                    ...
                    $forceUnlimited = $produc['force_unlimited'];
                    $isUnlimited = ($price_sale >= $actualPrice) || $forceUnlimited;
                    $classEntity = null;

                    $produc_advancement = 0;
                    ...
                    if ($produc['mode'] == 3) {
                        ...
                        // Sumar avance de pago
                        $registration->advancement = ($registration->advancement ?? 0) + $price_sale;
                        $produc_advancement = ($registration->advancement ?? 0) + $price_sale;
                        ...
                        if ($isUnlimited) {
                            ...
                            $registration->next_payment_date = $nextPaymentDate;
                        }
                        ...
                        if (!$studentSubscription || !($studentSubscription instanceof AcaStudentSubscription)) {
                            throw new \Exception("No se encontró la suscripción del alumno o no es un modelo.");
                        }


                        // Sumar avance
                        $studentSubscription_advancement = ($studentSubscription->advancement ?? 0) + $price_sale;
                        $produc_advancement = ($studentSubscription->advancement ?? 0) + $price_sale;

                        $studentSubscription_date_end = null;
                        $studentSubscription_status = true;
                        ...
                        if ($isUnlimited) {
                            ...
                            $studentSubscription_date_end = $dateEnd;
                        } else {
                            $studentSubscription_date_end = $nextPaymentDate;
                        }

                        // Asociar documento
                        $studentSubscription_xdocument_id = $document->id;

                        AcaStudentSubscription::where('student_id', $student_id)
                            ->where('subscription_id', $product_id)
                            ->update([
                                'advancement' => $studentSubscription_advancement,
                                'date_end'   => $studentSubscription_date_end,
                                'xdocument_id' => $studentSubscription_xdocument_id,
                                'status' => $studentSubscription_status
                            ]);
                    }

                    SaleProduct::find($produc['originId'])
                        ->update([
                            'advancement' => $produc_advancement
                        ]);
