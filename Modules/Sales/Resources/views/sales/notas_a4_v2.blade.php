    ...
    <div class="page">
        <header class="example-header clearfix">
            ...
            <div id="company">
                ...
                <div>{{ $document->getCompany()->getAddress()->getDepartamento() }}-{{ $document->getCompany()->getAddress()->getProvincia() }}-{{ $document->getCompany()->getAddress()->getDistrito() }}</div>
                <div>{{ env('APP_URL') }}</div>
            </div>
        </header>
        ...
        <main>
            <div id="details" class="example-details clearfix">
                <div id="client">
                    ...
                    <div class="line">{{ $document->getTipDocAfectado() == '01' ? 'RUC' : 'Documento' }}: {{ $document->getClient()->getNumDoc() }}</div>
                    <div class="line">{{ $clientAddressText }}</div>
                    <div class="line">Documento afectado: {{ $document->getNumDocfectado() }}</div>
                </div>
                <div id="invoice">
                    <h1>{{ $document->getTipoDoc() == '07' ? 'NOTA DE CREDITO ELECTRÓNICA' : 'NOTA DE DEBITO ELECTRÓNICA' }}</h1>
                    <div class="date">Nro: {{ $document->getSerie() }}-{{ $document->getCorrelativo() }}</div>
                    <div class="date">Fecha: {{ $document->getFechaEmision()->format('d/m/Y') }}</div>
                    <div class="date">Modifica: {{ $document->getTipDocAfectado() == '01' ? 'FACTURA' : 'BOLETA DE VENTA' }}</div>
                </div>
            </div>

            <div class="notice-card" style="margin-bottom: 18px;">
                <div class="title">MOTIVO DE EMISION</div>
                <div class="body">
                    Fecha del documento afectado: {{ $document->getFechaDocAfectado()->format('d/m/Y') }}<br>
                    Motivo de la nota de {{ $document->getTipoDoc() == '07' ? 'crédito' : 'débito' }}:
                    <strong>{{ $document->getDesTypeMotivo() }}</strong>
                </div>
            </div>
